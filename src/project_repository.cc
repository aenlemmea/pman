...
ProjectRepository::ProjectRepository(const std::filesystem::path &root)
...
void ProjectRepository::save(const Project &proj) {
  ...
  auto jfp = get_path() / proj.get_name();
  jfp += ".json";
...
Project ProjectRepository::load(const std::string_view name) const {
  ...
  nlohmann::json j;
  is >> j;
  Project p{name, Language::UNKNOWN, get_path() / name};
  from_json(j, p);
  return p;
}
